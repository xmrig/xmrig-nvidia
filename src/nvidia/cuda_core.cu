...
#define VARIANT1_1(p) \
  ...
  { \
    ...
    /* ((uint8_t*)p)[11-8] = (tmp & 0xef) | (tmp0<<4); */ \
...
template<size_t ITERATIONS, size_t OFFSET, size_t MASK>
void cryptonight_core_cpu_hash(nvid_ctx* ctx, int variant, uint8_t nonce_flag)
{
...
extern "C" void cryptonight_gpu_hash(nvid_ctx* ctx, int variant, uint8_t nonce_flag)
{
    cryptonight_core_cpu_hash<0x80000, 19, 0x1FFFF0>(ctx, variant, nonce_flag);
}
...
{
    cryptonight_core_cpu_hash<0x40000, 18, 0x0FFFF0>(ctx, 0, 0);
}
#endif
