...
void xmrig::Network::onActive(IStrategy *strategy, Client *client)
{
    ...
    LOG_INFO(WHITE_BOLD("use pool ") CYAN_BOLD("%s:%d ") GREEN_BOLD("%s") " " BLACK_BOLD_S "%s",
             client->host(), client->port(), tlsVersion ? tlsVersion : "", client->ip());

    const char *fingerprint = client->tlsFingerprint();
    if (fingerprint != nullptr) {
        LOG_INFO(BLACK_BOLD_S "fingerprint (SHA-256): \"%s\"", fingerprint);
    }
}
...
void xmrig::Network::onResultAccepted(IStrategy *, Client *, const SubmitResult &result, const char *error)
{
    ...
    if (error) {
        LOG_INFO(RED_BOLD("rejected") " (%" PRId64 "/%" PRId64 ") diff " WHITE_BOLD("%u") " " RED("\"%s\"") " " RED_BOLD("(%" PRIu64 " ms)"),
